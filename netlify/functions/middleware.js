...
// FIX v53: Usa firebase-init.js como source of truth — evita double-init race condition
...
/**
 * assertTenantAccess — verifica rapidamente se uid pertence ao tenantId.
 * Usado por endpoints com skipTenant:true que ainda precisam de isolamento.
 * Master bypassa. Retorna null se ok, ou resposta 403 se negado.
 */
async function assertTenantAccess(event, tenantId, headers) {
  const uid  = event._uid;
  const role = event._role;
